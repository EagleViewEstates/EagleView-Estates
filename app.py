...
def send_email(data):
    try:
        ...
        password = st.secrets["EMAIL_PASSWORD"] # See execution guide below
...
st.markdown("""
    ...
    /* Building-Out Success Container */
    .success-overlay {
        position: fixed;
    ...
    /* Stylized Eagle SVG with Gold Build-Out */
    .eagle-build-out {
...
<div class='anchor-card'>
    <p style='margin:0; font-size: 0.8em; color: #888;'>PREMIUM OPPORTUNITY</p>
...
if os.path.exists("site_photo.jpg"):
    st.image("site_photo.jpg", caption="Red Fife Road Staging Area", use_column_width=True)
else:
    st.info("Photo Placeholder: site_photo.jpg")
...
with st.form("anchor_survey", clear_on_submit=True):
    ...
    q2 = st.select_slider("2. Strategic Value of CentrePort Winnipeg Location", options=["Low", "Neutral", "Important", "Strategic", "Critical"])
    q3 = st.radio("3. Target Deployment Date", ["June 1st", "Summer 2026", "Fall/Winter 2026"])
    q4 = st.multiselect("4. High-Priority Amenities", ["Biometric/Coded Gate", "LED Lighting", "Live CCTV Feed", "Snow Removal", "Grading Support"])
    st.markdown("---")
    q5 = st.radio("5. Secure your position with a 'Pre-Lease' agreement prior to the June 1st launch?", ["YES - Contact with terms", "MAYBE - Send pricing details", "NO - On-demand only"])
    st.divider()
